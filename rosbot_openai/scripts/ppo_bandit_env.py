...
register(
    ...
    entry_point='ppo_bandit_env:HusarionWalldodgeEnv',
...
class HusarionWalldodgeEnv(husarion_env.HusarionEnv):
    ...
    def _set_init_pose(self):
        ...
        self.total_runs += 1.0
        #print("TASKNUM: %s", self.tasknumber)
        #self.show_goal_pos()
        ...
        file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/tasks/taskfile.txt", "r")
        self.tasknumber = int(file.read())
        file.close()
        print("NUM: %s", self.tasknumber)
        if self.tasknumber==0:
            self.model = self.build_model("/home/marvin/model_editor_models/diff_0/model.sdf")
        elif self.tasknumber==1:
            self.model = self.build_model("/home/marvin/model_editor_models/diff_1/model.sdf")
        elif self.tasknumber==2:
            self.model = self.build_model("/home/marvin/model_editor_models/diff_2/model.sdf")
        elif self.tasknumber==3:
            self.model = self.build_model("/home/marvin/model_editor_models/diff_3/model.sdf")
    ...
    def _is_done(self, observations):
        ...
        if is_done:
            ...
            if self.tasknumber==0:
                ...
                self.update_desired_pos(rand_x, rand_y)
            ###########################################################################
            # TASK diff_1:
            ###########################################################################
            elif self.tasknumber==1:
            ...
            ###########################################################################
                ...
                if rand_number == 0:
                    ...
                    self.update_desired_pos(3.0, -6.0)
                elif rand_number == 12:
                    self.update_desired_pos(4.0, 6.0)
                elif rand_number == 13:
