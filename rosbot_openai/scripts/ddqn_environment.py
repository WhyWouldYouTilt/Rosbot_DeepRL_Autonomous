...
class HusarionWalldodgeEnv(husarion_env.HusarionEnv):
    def __init__(self):
        ...
        number_actions = 13#rospy.get_param('/husarion/n_actions')
        self.action_space = spaces.Discrete(number_actions)

        #Obs_shape for 48 Lasescan
        high_laser = numpy.full(48, 1.0)
        low_laser = numpy.full(48, 0.0)
        #Obs_shape for previous actions
        high_act = numpy.array([1.0,1.0])
        low_act = numpy.array([0.0,-1.0])
        #Obs_shape for distance
        high_polar = numpy.array([1.0,1.0])
        low_polar = numpy.array([0.0,0.0])
        #Obs_shape for diffangle
        high_diff = numpy.array([1.0])
        low_diff = numpy.array([0.0])
        high = numpy.concatenate([high_laser, high_act, high_polar, high_diff])
        low = numpy.concatenate([low_laser, low_act, low_polar, low_diff])
        self.observation_space = spaces.Box(low, high)
        print("SHAPE: %s", self.observation_space)
        ...
        self.max_laser_value = 5.5
        self.min_laser_value = 0.04
        ...
        self.work_space_y_max = 100.0  # rospy.get_param("/husarion/work_space/y_max")
    ...
    def _get_obs(self):
        ...
        observations = discretized_laser_scan + [past_lin] + [past_ang] + [current_distance/diagonal_dis] + [rel_theta / 360] + [diff_angle]
    ...
    def _compute_reward(self, observations, done):
        ...
        if not done:
            ...
            if distance_difference>0:
                ...
                reward =-1#reward = (self.closer_to_point_param * distance_difference)+ (self.angle_reward * diff_angle_diff)
        else:
