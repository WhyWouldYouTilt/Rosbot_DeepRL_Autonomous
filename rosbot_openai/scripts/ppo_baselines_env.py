...
register(
    ...
    entry_point='ppo_baselines_env:HusarionWalldodgeEnv',
...
class HusarionWalldodgeEnv(husarion_env.HusarionEnv):
    def __init__(self):
        ...
        number_actions = 5#rospy.get_param('/husarion/n_actions')
        self.action_space = spaces.Discrete(number_actions)

        #Obs_shape for 48 Lasescan
        high_laser = numpy.full(28, 5)
        low_laser = numpy.full(28, 0.0)
        #Obs_shape for distance
        high_polar = numpy.array([1.0,1.0,1.0,1.0])
        low_polar = numpy.array([0.0,0.0,0.0,0.0])
        high = numpy.concatenate([high_laser, high_polar])
        low = numpy.concatenate([low_laser, low_polar])
    ...
    def _get_obs(self):
        ...
        observations = discretized_laser_scan + [current_distance/diagonal_dis] + [rel_theta/360] + [yaw] + [diff_angle]
        return observations
    ...
    def _is_done(self, observations):
        ...
        # Its all the array except from the last four elements
        laser_readings = observations[:-4]
