...
from eps_bandit import Tasks
import ppo_bandit_env
...
parser.add_argument('--ckpt_folder', default='/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/models', help='Location to save checkpoint models')
parser.add_argument('--tb', default=False, action='store_true', help='Use tensorboardX?')
parser.add_argument('--log_folder', default='/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs', help='Location to save logs')
...
def train(env_name, env, state_dim, action_dim, render, solved_reward,
    ...
    ckpt = '/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/models/PPO_continuous_500000_Steps_7x7_obstacles_2.pth'
    if restore:
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        if p < eps:
            ...
            j = np.argmax([t.mean for t in tasks])
        ...
        # Change the task to be loaded for the environment. The number of the task is stored in "taskfile.txt" and loaded in the ppo_bandit_env.py class
        file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/tasks/taskfile.txt", "w")
        file.write(str(j))
        file.close()
        #Reset state and call environment to build the correct stage
        state = env.reset()
        #Start of an Epoch:
        #In this loop the actions of an epoch get determined and executed by the policy.
        for t in range(max_timesteps):
            ...
            if total_time_step % 100000 == 0:
                torch.save(ppo.policy.state_dict(), ckpt_folder + '/PPO_continuous_{}_Steps_curriculum_eps_greedy.pth'.format(total_time_step))

            #Log everything every 10k timesteps so you can plot it in another file
            if total_time_step % 10000 == 0:
                ...
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/total_reward.txt", "a")
                file.write(str(int(total_reward)) + ",")
                file.close()
                total_reward = 0
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/total_runs.txt", "a")
                file.write(str(runs_total) + ",")
                file.close()
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/succ_runs.txt", "a")
                file.write(str(runs_succ) + ",")
                file.close()
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/collisions.txt", "a")
                file.write(str(runs_collisions) + ",")
                file.close()
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/task_0_count.txt", "a")
                file.write(str(task_0_count) + ",")
                file.close()
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/task_1_count.txt",
                            "a")
                file.write(str(task_1_count) + ",")
                file.close()
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/task_2_count.txt",
                            "a")
                file.write(str(task_2_count) + ",")
                file.close()
                file = open("/home/marvin/ros_workspace/src/rosbot_openai/ppo_continuous_bandit/logs/task_3_count.txt",
                            "a")
                file.write(str(task_3_count) + ",")
                file.close()
        ...
        #Computes the reward for each episode. Is used as the progress signal for the multi armed bandit
        #episode_reward /= episode_steps
        #Need to decide which was the latest task, to get the correct progress for every task
        if j == 0:
            progress_signal = episode_reward-last_0_reward
            last_0_reward = episode_reward
            if first_run_0>=2:
                tasks[j].update(progress_signal)
        elif j == 1:
            progress_signal = episode_reward - last_1_reward
            last_1_reward = episode_reward
            if first_run_1>=2:
                tasks[j].update(progress_signal)
        elif j == 2:
            progress_signal = episode_reward - last_2_reward
            last_2_reward = episode_reward
            if first_run_2>=2:
                tasks[j].update(progress_signal)
        elif j == 3:
            progress_signal = episode_reward - last_3_reward
            last_3_reward = episode_reward
            if first_run_3>=2:
                tasks[j].update(progress_signal)


        print("####################################################################################")
        print("Mean 0: %s", tasks[0].mean)
        print("Mean 1: %s", tasks[1].mean)
        print("Mean 2: %s", tasks[2].mean)
        print("Mean 3: %s", tasks[3].mean)
        print("####################################################################################")
        print("PROGRESS: %s", progress_signal)
        avg_length += t
