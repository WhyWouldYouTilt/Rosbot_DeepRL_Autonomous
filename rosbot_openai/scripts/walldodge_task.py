...
register(
    id='Husarion_Walldodge-v0',
    entry_point='walldodge_task:HusarionWalldodgeEnv',
...
class HusarionWalldodgeEnv(husarion_env.HusarionEnv):
    def __init__(self):
        ...
        #Getting only 180 degrees of LaserScan
        laser_scan_180 = self._check_laser_scan_180_ready()
        num_laser_readings = len(laser_scan_180.ranges) / self.new_ranges
        high_laser_180 = numpy.full((num_laser_readings), self.max_laser_value)
        low_laser_180 = numpy.full((num_laser_readings), self.min_laser_value)
        ...
        high_180 = numpy.concatenate([high_laser_180, high_odometry, high_des_pos])
        low_180 = numpy.concatenate([low_laser_180, low_odometry, low_des_pos])

        self.observation_space = spaces.Box(low, high)
        self.observation_space_180 = spaces.Box(low_180, high_180)
        ...
        self.closer_to_point_param = rospy.get_param("/husarion/closer_to_point_param")
    ...
    '''def _set_action(self, action):
        ...
        #Turning Left in different degrees
        elif action == 1: #LEFT
    ...
    def _set_action(self, action):
        ...
        # Turning Right in different degrees'''
            ...
            self.rotate(self.angular_speed, 10.0, True)
            last_action = "10 Degree Clockwise"
        elif action == 2:
            self.rotate(self.angular_speed, 20.0, True)
            last_action = "20 Degree Clockwise"
        elif action == 3:
            self.rotate(self.angular_speed, 30.0, True)
            last_action = "30 Degree Clockwise"
        elif action == 4:
            self.rotate(self.angular_speed, 40.0, True)
            last_action = "40 Degree Clockwise"

            '''elif action == 5:
            self.rotate(self.angular_speed, 50.0, True)
            last_action = "50 Degree Clockwise"
        elif action == 6:
            self.rotate(self.angular_speed, 60.0, True)
            last_action = "60 Degree Clockwise"
        elif action == 7:
            self.rotate(self.angular_speed, 70.0, True)
            last_action = "70 Degree Clockwise"
        elif action == 8:
            self.rotate(self.angular_speed, 80.0, True)
            last_action = "80 Degree Clockwise"
        elif action == 9:
            self.rotate(self.angular_speed, 90.0, True)
            last_action = "90 Degree Clockwise"'''
            # Turning Left every 10 degrees
        elif action == 5:
            self.rotate(self.angular_speed, 10.0, False)
            last_action = "10 Degree Not Clockwise"
        elif action == 6:
            self.rotate(self.angular_speed, 20.0, False)
            last_action = "20 Degree Not Clockwise"
        elif action == 7:
            self.rotate(self.angular_speed, 30.0, False)
            last_action = "30 Degree Not Clockwise"
        elif action == 8:
            self.rotate(self.angular_speed, 40.0, False)
            last_action = "40 Degree Not Clockwise"
        #rospy.logdebug("END Set Action ==>" + str(action) + ", ACTION=" + str(last_action))
    ...
    def _get_obs(self):
        ...
        laser_scan_180 = self.get_laser_scan_180()
        discretized_laser_scan = self.discretize_scan_observation(laser_scan,
                                                                  self.new_ranges
                                                                  )
        discretized_laser_scan_180 = self.discretize_scan_observation(laser_scan_180,
                                                                  self.new_ranges
                                                                  )
        ...
        observations_180 = discretized_laser_scan_180 + odometry_array + desired_position

        # rospy.logwarn("Observations==>"+str(observations))
        # rospy.logwarn("END Get Observation ==>")
        #return observations_180
        return observations
    ...
    def _is_done(self, observations):
        ...
        laser_readings_180 = observations[:-5]
